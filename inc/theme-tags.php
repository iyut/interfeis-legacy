...
if ( ! function_exists( 'ifs_legacy_print_posted_on' ) ) :
	...
	function ifs_legacy_print_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'ifs-legacy' ),
	...
	function ifs_legacy_print_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'ifs-legacy' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'ifs-legacy' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
