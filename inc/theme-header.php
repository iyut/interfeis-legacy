...
if(!function_exists("ifs_legacy_searchform")){
	function ifs_legacy_searchform($id="", $class=""){
        ...
        if( taxonomy_exists('product_cat')){
            $selectproductcat = wp_dropdown_categories(array(
		...
		$output = '<div class="'. esc_attr( $class ).'">';
			$output .= '<form method="get" id="'.esc_attr( $id ).'" name="'.esc_attr( $id ).'" class="btntoppanel" action="'. esc_url( home_url( '/' ) ) .'">';
				$output .= '<input type="submit" class="submit" name="submit" value="" />';
				$output .= '<button type="submit" class="submittext" name="submit"><i class="fa fa-search"></i> <span>'. esc_html__('Search', 'ifs-legacy') .'</span></button>';
				$output .= '<div class="searcharea">';
					$output .= '<input type="text" name="s" autocomplete="off" class="txtsearch" placeholder="'. esc_attr( $searchtext ) .'" value="" />';
					$output .= $selectproductcat;
                    $output .= $outputposttype;
					$output .= '<a href="#" class="searchclose"></a>';
				$output .= '</div>';
			$output .= '</form>';
		$output .= '</div>';
...
if(!function_exists("ifs_legacy_minicart")){
	function ifs_legacy_minicart($id="",$class=""){
		...
		$output = '<div id="'.esc_attr( $id ).'" class="'.esc_attr( $class ).'">';
			$output .= '<a class="topcartbutton btnpanel" href="'.esc_url( $link ).'"><i class="shopicon fa fa-shopping-bag"></i> <span class="topbtn_text">'.esc_html__( "My Cart", "ifs-legacy" ).'</span><span class="cart_subqty"><i class="cart_totalqty">'.$totalqty.'</i><i class="cart_subtotal">'.$cart_subtotal.'</i></span><div class="clearfix"></div></a>';
		...
		echo apply_filters('ifs_legacy_minicart', $output );
	}
}

/**
 * return register form string
 *
...
if(!function_exists('ifs_legacy_register_form')){
	function ifs_legacy_register_form() {
        ...
        $disable_regtext = ifs_legacy_get_option('ifs_legacy_disable_regsuccesstext');
        if($disable_regtext!=true){
            $redirectregister = add_query_arg("nvrsuccessregtext", $successtext, $redirectregister);
        }

        $return_string='
            <div class="login_form">
            <form id="registerform" name="registerform" action="'.esc_url( wp_registration_url() ).'" method="post">
				   <div class="loginalert" id="register_message_area" ></div>

					<p><label for="user_login_register">'.esc_html__('Username', "ifs-legacy" ).'</label><input type="text" name="user_login" id="user_login_register" class="textbox" value="" size="20" /></p>
					<p><label for="user_email_register">'.esc_html__('Email', "ifs-legacy" ).'</label><input type="text" name="user_email" id="user_email_register" class="textbox" value="" size="20" /></p>

					<p id="reg_passmail">'.esc_html__('A password will be e-mailed to you', "ifs-legacy" ).'</p>';
        ...
        $return_string .= '
          <input type="hidden" name="redirect_to" value="'.esc_url( $redirectregister ).'" />
					<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button" value="'.esc_attr__('Register', "ifs-legacy" ).'" /></p>
            </form>
			</div>

		';
		 echo apply_filters('ifs_legacy_register_form', $return_string);
	}
}

/**
 * return login form string
 *
 * @uses ifs_legacy_login_form()
 */
	...
	function ifs_legacy_login_form() {
     ...
     $login_args = array(
       'echo' => false,
		...
		$return_string='<div class="login_form" id="ifs-login-div">';
			$return_string .= wp_login_form( $login_args );
		  $return_string .= '<div class="login-links">';
			   $return_string.='<a href="'. esc_url( wp_lostpassword_url() ) .'" id="forgot_pass">'.esc_html__('forgot password?', "ifs-legacy" ).'</a>';
			$return_string .= '</div>';
		$return_string .= '</div>';

		echo apply_filters('ifs_legacy_login_form', $return_string);
	}
}
